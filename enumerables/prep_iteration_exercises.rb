...
def factors(num)
  result = []
  (1..num).each { |factor| result << factor if num % factor == 0 }
  result
end
  ...
  def bubble_sort!(&prc)
    ...
    while !sorted
      ...
      (0...self.length-1).each do |i|
        ...
        # to use with a given block
        if prc.call(self[i],self[i+1]) == 1
            self[i], self[i+1] = self[i+1], self[i]
            sorted = false
        end

        # standard bubble sort sorting
        # if self[i] > self[i+1]
        #   self[i], self[i+1] = self[i+1], self[i]
        #   sorted = false
        # end
...
# OXFORD = ["ban","nana","ana","an","a"]
# p subwords("banana",OXFORD)
...
# arr0 = [3,4,5]
# p arr0.my_inject { |res,el| res+el }
...
def concatenate(strings)
  strings.inject {|accum,el| accum + el }
end